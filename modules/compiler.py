...
class CompilationAgent:
    ...
    def process_paper(self, paper_metadata):
        ...
        pdf_basename = os.path.splitext(os.path.basename(pdf_path))[0]
        print(f"Processing: {paper_metadata.get('title', 'Unknown')}")
    ...
    def extract_sections_text(self, pdf_path):
        ...
        # Process lines for section detection
        lines = all_text.split('\n')
        i = 0
        ...
        while i < len(lines):
            ...
            # Skip references content
            if current_section.lower() == 'references' and not self.is_potential_header(line, font_info, i, header_threshold):
                i += 1
                continue
            ...
            if numbered_match:
                if text_accum:
                    sections[current_section] = self.clean_text(" ".join(text_accum))
                    text_accum = []
            ...
            if self.is_potential_header(line, font_info, i, header_threshold):
                ...
                if (2 <= len(words) <= 8 and 
                    ...
                    not any(word.lower() in ['the', 'and', 'or', 'but', 'however', 'therefore'] for word in words[:3])):
    ...
    def is_potential_header(self, line, font_info, line_index, threshold):
    ...
    def merge_small_sections(self, sections, min_words=30):
    ...
    def summarize_section(self, text):
        """Summarize a section with Ollama using chunking + generic prompt."""
        text = text.strip()
        if not text or len(text.split()) < 30:
            return text
...
"""
    ...
    def check_ollama_connection(self):
        ...
        try:
            ...
            print("💡 Make sure to run 'ollama serve' in a separate terminal")
            return False
